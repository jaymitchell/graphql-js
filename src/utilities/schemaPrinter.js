...
function printEnumValues(values): string {
  return values.map(value =>
    printDescription(value, '  ') + '  ' + value.name + printDeprecated(value)
  ).join('\n');
}
...
function printInputObject(type: GraphQLInputObjectType): string {
  ...
  return printDescription(type) +
    `input ${type.name} {\n` +
      fields.map(f =>
        printDescription(f, '  ') + '  ' + printInputValue(f)
      ).join('\n') + '\n' +
    '}';
}
...
function printFields(type) {
  ...
  const fields = Object.keys(fieldMap).map(fieldName => fieldMap[fieldName]);
  return fields.map(f =>
    printDescription(f, '  ') + '  ' + f.name + printArgs(f.args, '  ') + ': ' +
...
function printArgs(args, indentation = '') {
  ...
  return '(\n' + args.map(arg =>
    printDescription(arg, '  ' + indentation) + '  ' + indentation +
    printInputValue(arg)
  ).join('\n') + '\n' + indentation + ')';
}
...
function printDescription(def, indentation = ''): string {
  if (!def.description) {
    return '';
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i] === '') {
      ...
      // For long lines, cut at space boundaries into sublines of ~100 chars.
      const sublines = breakLine(lines[i], 98 - indentation.length);
...
function breakLine(line: string, len: number): Array<string> {
  ...
  const parts = line.split(new RegExp(`((?: |^).{15,${len - 10}}(?= |$))`));
  if (parts.length < 4) {
    return [ line ];
  }
